...
namespace Hoshino17
{
	public sealed class H17CubemapGeneratorEditorMainTab : H17CubemapGeneratorEditorTabBase
	{
		...
		public H17CubemapGeneratorEditorMainTab(H17CubemapGeneratorEditorContext context, IH17CubemapGeneratorEditor editor)
			: base(context, editor)
		{
		...
		public override void OnGUI()
		{
			...
			{
				context.inputSource = EnumUtils.EnumByIndex<H17CubemapGenerator.InputSource>(inputSourceIndex);
			}
			...
			switch (context.inputSource)
			{
				case H17CubemapGenerator.InputSource.CurrentScene:
					...
					if (EditorGUI.EndChangeCheck())
					{
						context.textureWidth = EnumUtils.EnumByIndex<H17CubemapGeneratorEditorContext.TextureWidthType>(textureWidthIndex);
					}
					...
					if (EditorGUI.EndChangeCheck())
					{
						context.horizontalRotation = EnumUtils.EnumByIndex<H17CubemapGeneratorEditorContext.RotationAngleType>(anglesIndex);
					}
					break;
			...
			if (EditorGUI.EndChangeCheck())
			{
				context.outputLayout = EnumUtils.EnumByIndex<H17CubemapGenerator.OutputLayout>(outputLayoutIndex);
			}
			...
			EditorGUI.BeginDisabledGroup(!H17CubemapGenerator.IsOutputLayoutAvailableBeCubemap(context.outputLayout));
