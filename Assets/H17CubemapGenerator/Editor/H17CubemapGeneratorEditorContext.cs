...
namespace Hoshino17
{
	public sealed class H17CubemapGeneratorEditorContext
	{
		...
		readonly PropertyInputSource _inputSourceProp = new PropertyInputSource(SettingsKeys.InputSource, H17CubemapGenerator.InputSource.CurrentScene);
		readonly PropertyOutputLayout _outputLayoutProp = new PropertyOutputLayout(SettingsKeys.OutputLayout, H17CubemapGenerator.OutputLayout.CrossHorizontal);
		...
		public class PropertyInputSource : PropertyBase<H17CubemapGenerator.InputSource>
		{
			public PropertyInputSource(string? prefsKey, H17CubemapGenerator.InputSource def = default(H17CubemapGenerator.InputSource))
				: base(prefsKey, def,
				onLoadValue: (prefsKey_, def_) => (H17CubemapGenerator.InputSource)PropertyKeyValue.LoadInt(prefsKey_, (int)def_),
				onSaveValue: (prefsKey_, value_) => PropertyKeyValue.SaveInt(prefsKey_, (int)value_))
			{ }
			protected override bool Equals(H17CubemapGenerator.InputSource other) => _value == other;
		}

		public class PropertyOutputLayout : PropertyBase<H17CubemapGenerator.OutputLayout>
		{
			public PropertyOutputLayout(string? prefsKey, H17CubemapGenerator.OutputLayout def = default(H17CubemapGenerator.OutputLayout))
				: base(prefsKey, def,
				onLoadValue: (prefsKey_, def_) => (H17CubemapGenerator.OutputLayout)PropertyKeyValue.LoadInt(prefsKey_, (int)def_),
				onSaveValue: (prefsKey_, value_) => PropertyKeyValue.SaveInt(prefsKey_, (int)value_))
			{ }
			protected override bool Equals(H17CubemapGenerator.OutputLayout other) => _value == other;
		}
