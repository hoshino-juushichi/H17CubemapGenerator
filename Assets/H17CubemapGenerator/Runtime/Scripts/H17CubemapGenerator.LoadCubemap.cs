...
namespace Hoshino17
{
	public partial class H17CubemapGenerator : MonoBehaviour, IH17CubemapGenerator
	{
...
#if UNITY_EDITOR
		public void SetSourceCubamap(Cubemap cubamap)
		{
		...
		void VetifyAndSetupLoadCubemap()
		{
		...
		void LoadCubemapFacesFromCubemap(Cubemap cubemap)
		{
			...
			_isSourceHDR = IsHDRFormat(cubemap.format);
			...
			bool isLinear = !_isSourceHDR;
#if UNITY_2022_1_OR_NEWER
			isLinear = !cubemap.isDataSRGB;
#endif
			for (int i = 0; i < 6; i++)
			{
				var faceIndex = (CubemapFace)i;
				Color[] pixels = readableCubeMap.GetPixels(faceIndex, 0);
